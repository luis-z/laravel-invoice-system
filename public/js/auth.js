...
function auth () {
  var cipher = new Cipher('EjercicioLaravel')
  var encripted = localStorage.getItem('ApiKey')
  var descifrado = cipher.descifrar(encripted)

  const config = {
    headers: {
      Accept: 'application/json',
      Authorization: 'Bearer ' + descifrado
    }
  }
  return config
}
...
function storeKey (ApiKey) {
  var cipher = new Cipher('EjercicioLaravel')
  var key = ApiKey
  var cifrado = cipher.cifrar(key)
  // var descifrado = cipher.descifrar(cifrado)
  localStorage.setItem('ApiKey', cifrado)
}
